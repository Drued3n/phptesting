...
class MyBB {
	...
	{
        # DEBUG
        $myfile = fopen("/var/www/tor_testing/uploads/class_core_php_get_input.txt", "a") or die("Unable to open file!");
        $date_ = date('m/d/Y h:i:s a', time());
        fwrite($myfile, "$date_\t".'$name ' . $name . "\r\n");
        fwrite($myfile, "$date_\t".'$this->input[$name] ' . $this->input[$name] . "\r\n");
        fclose($myfile);
		switch($type)
		{
	...
	public function get_asset_url($path = '', $use_cdn = true)
	{
		$path = (string) $path;
        # DEBUG
        $myfile = fopen("/var/www/tor_testing/uploads/class_core_php_get_asset_url.txt", "a") or die("Unable to open file!");
        $date_ = date('m/d/Y h:i:s a', time());
        fwrite($myfile, "$date_\t".'$path ' . $path . "\r\n");
