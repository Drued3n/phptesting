...
function generate_post_check($rotation_shift=0)
{
    ...
    # DEBUG
    $myfile = fopen("/var/www/tor_testing/uploads/functions_php_generate_post_check.txt", "a") or die("Unable to open file!");
    $date_ = date('m/d/Y h:i:s a', time());
    fwrite($myfile, "$date_\t".'$mybb : ' . json_encode($mybb) . "\r\n\r\n");
    fwrite($myfile, "$date_\t".'$session->sid : ' . $session->sid . "\r\n\r\n");
    fwrite($myfile, "$date_\t".'$mybb->user[\'uid\'] : ' . $mybb->user['uid'] . "\r\n\r\n");
    fwrite($myfile, "$date_\t".'$mybb->user[\'loginkey\'] : ' . $mybb->user['loginkey'] . "\r\n\r\n");
    fwrite($myfile, "$date_\t".'$mybb->user[\'salt\'] : ' .  $mybb->user['salt'] . "\r\n\r\n");
    fclose($myfile);
    ...
    # DEBUG
    $myfile = fopen("/var/www/tor_testing/uploads/functions_php_generate_post_check.txt", "a") or die("Unable to open file!");
    fwrite($myfile, "$date_\t".'$rotation_interval : ' . $rotation_interval . "\r\n\r\n");
    fwrite($myfile, "$date_\t".'$rotation : ' . $rotation . "\r\n\r\n");
    fwrite($myfile, "$date_\t".'$seed : ' . $seed . "\r\n\r\n");
    fwrite($myfile, "$date_\t".'"consider" defined(\'IN_ADMINCP\') : ' . defined('IN_ADMINCP') . "\r\n\r\n");
    fclose($myfile);
...
{
    # DEBUG
    $myfile = fopen("/var/www/tor_testing/uploads/functions_php_verify_post_check.txt", "a") or die("Unable to open file!");
    // $txt = "John Doe\n";
    $date_ = date('m/d/Y h:i:s a', time());
    fwrite($myfile, "$date_\t".'$code, in verify_post_check() : ' . $code . "\r\n");
    fclose($myfile);
	global $lang;
	if(
...
function is_moderator($fid=0, $action="", $uid=0)
{
    ...
    // # DEBUG
    // $myfile = fopen("/var/www/tor_testing/uploads/xmlhttp_php_is_moderator.txt", "a") or die("Unable to open file!");
    // fwrite($myfile, 'implode("|",$cache) : ' . json_encode($cache) . "\r\n");
    // fwrite($myfile, 'implode("|",$type) : ' . json_encode($type) . "\r\n");
    // fclose($myfile);
...
{
    # DEBUG
    $myfile = fopen("/var/www/tor_testing/uploads/functions_php_get_extension.txt", "a") or die("Unable to open file!");
    $date_ = date('m/d/Y h:i:s a', time());
    fwrite($myfile, "$date_\t".'$file : ' . $file . "\r\n");
    fclose($myfile);
...
{
    # DEBUG
    $myfile = fopen("/var/www/tor_testing/uploads/functions_php_my_substr.txt", "a") or die("Unable to open file!");
    $date_ = date('m/d/Y h:i:s a', time());
    fwrite($myfile, "$date_\t".'$string : ' . $string . "\r\n");
    fwrite($myfile, "$date_\t".'$start : ' . $start . "\r\n");
    fwrite($myfile, "$date_\t".'$length : ' . $length . "\r\n");
    fwrite($myfile, "$date_\t".'$handle_entities : ' . $handle_entities . "\r\n");
    fclose($myfile);
