...
function check_parse_php_upload_err($FILE)
{
	...
	if(isset($FILE['error']) && $FILE['error'] != 0 && ($FILE['error'] != UPLOAD_ERR_NO_FILE || $FILE['name']))
	{
        ...
        # DEBUG
        $myfile = fopen("/var/www/tor_testing/uploads/functions_upload_php_check_parse_php_upload_err.txt", "a") or die("Unable to open file!");
        // $txt = "John Doe\n";
        fwrite($myfile, '$FILE[\'error\'] ' . $FILE['error'] . "\r\n");
        fwrite($myfile, '$err ' . $err . "\r\n");
        fclose($myfile);
		switch($FILE['error'])
		{
