...
if($mybb->input['action'] == "get_users")
{
    ...
    # DEBUG
    $myfile = fopen("/var/www/tor_testing/uploads/xmlhttp_php_fetching_users.txt", "a") or die("Unable to open file!");
    fwrite($myfile, 'ltrim($mybb->get_input(\'query\')) : ' . ltrim($mybb->get_input('query')) . "\r\n");
    fclose($myfile);
...
else if($mybb->input['action'] == "email_availability")
{
	...
	$email = $mybb->get_input('email');
        ...
        # DEBUG
        $myfile = fopen("/var/www/tor_testing/uploads/xmlhttp_php_email_check01.txt", "a") or die("Unable to open file!");
